...
static void _finalize(dispatcher_context_t *dc) {
    ...
    bool ok = hkdf_derive_root(G_scratch.derive_ctx.app_name_buf,
                               ...
                               G_derive_streaming.context_total_len,
    ...
    memcpy(G_vault_context.derivation_path,
           ...
           G_derive_streaming.path_len * sizeof(uint32_t));
    G_vault_context.derivation_path_len = G_derive_streaming.path_len;
    ...
    memcpy(G_vault_context.app_name,
           G_scratch.derive_ctx.app_name_buf,
           G_derive_streaming.app_name_len);
    G_vault_context.app_name_len = G_derive_streaming.app_name_len;
...
static void handle_initial_chunk(dispatcher_context_t *dc, const command_t *cmd) {
    ...
    explicit_bzero(&G_derive_streaming, sizeof(G_derive_streaming));
    ...
    /* Store parsed fields: large buffers in G_scratch, small scalars in G_derive_streaming. */
    memcpy(G_scratch.derive_ctx.app_name_buf, app_name, app_name_len);
    G_derive_streaming.app_name_len = app_name_len;
    G_derive_streaming.path_len = path_len;
    G_derive_streaming.p2_mode = cmd->p2;
    G_derive_streaming.context_total_len = context_total_len;
    G_derive_streaming.context_received_len = (uint16_t) first_chunk_len;

    if (first_chunk_len > 0u) {
        memcpy(G_scratch.derive_ctx.context_buf, first_chunk, first_chunk_len);
    }

    if (G_derive_streaming.context_received_len < context_total_len) {
        /* More chunks expected — enter streaming mode. */
        G_derive_streaming.streaming_in_progress = true;
        SEND_SW(dc, SW_OK);
        return;
    }
...
static void handle_continuation_chunk(dispatcher_context_t *dc, const command_t *cmd) {
    ...
    if (cmd->p2 != G_derive_streaming.p2_mode) {
        SEND_SW(dc, SW_WRONG_P1P2);
        return;
    }
    ...
    uint16_t remaining =
        G_derive_streaming.context_total_len - G_derive_streaming.context_received_len;
    ...
    memcpy(G_scratch.derive_ctx.context_buf + G_derive_streaming.context_received_len, data, lc);
    G_derive_streaming.context_received_len += (uint16_t) lc;
