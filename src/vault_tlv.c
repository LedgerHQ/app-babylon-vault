...
vault_tlv_err_t vault_tlv_parse(const uint8_t *data, size_t len, vault_intent_t *out) {
    ...
    while (pos < len) {
        ...
        /* Reject tags outside the scalar range. */
        if (tag < 0x01 || tag > VAULT_INTENT_TAG_MAX) return VAULT_TLV_ERR_UNKNOWN_TAG;

        /* Reject duplicate tags. */
        uint32_t bit = VAULT_TAG_BIT(tag);
        ...
        switch (tag) {
            ...
            case TAG_DEPOSITOR_CLAIM_VALUE:
                if (field_len != 8) return VAULT_TLV_ERR_WRONG_LENGTH;
