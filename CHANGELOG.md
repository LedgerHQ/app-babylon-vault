...
- **NoPayout dedup replay nullifies the intent**: a dedup-slot collision returned `SW_INCORRECT_DATA` and left the session live, unlike the PegIn and Payout paths. Now invalidates the context and returns `SW_CAP_EXCEEDED`, per the HLD rule that any error in a signature flow nullifies the intent (`sign_psbt_validate.c`)
...
- **NoPayout per-challenger dedup bitmask**: added `nopayout_claimer_mask` (80-byte bitmask) to `vault_context_t`; the slot is checked during validation and committed *before* the signature yield, so a host that aborts mid-yield cannot replay the same NoPayout PSBT to exhaust the flat cap (`vault_context.h`, `sign_psbt_validate.c`, `sign_custom_inputs.c`)
- **NoPayout dedup slot corrected for multi-vault intents**: slot was `challenger_idx` alone, blocking the same challenger from signing for a second vault group. Slot is now `gi × (keeper_count + challenger_count) + challenger_idx`, where `gi` is found by matching Input 0's `PSBT_IN_PREVIOUS_TXID` against each group's computed PegIn txid (`vault_context.h`, `globals.c`, `sign_psbt_validate.c`, `sign_custom_inputs.c`)
