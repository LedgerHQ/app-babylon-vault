...
- **NoPayout dedup slot corrected for multi-vault intents**: slot was `challenger_idx` alone, blocking the same challenger from signing for a second vault group. Slot is now `gi × (keeper_count + challenger_count) + challenger_idx`; `gi` is inferred from a 4-byte fingerprint of the Assert:0 WITNESS_UTXO tweaked output key (`vault_context.h`, `globals.c`, `sign_psbt_validate.c`, `sign_custom_inputs.c`)
